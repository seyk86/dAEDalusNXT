...
%
function MeanAxisModal=simulate_free_flying_mean_axis_modal(aircraft,aircraft_structure,n_modes,trimmed_state,ctrl_input,t_vec,dt,maneuver_name,movie_on)
...
XHALE_aero=XHALE_aero.initialize_time_domain_solution(dt*rtf);
...
for n_ctrl=1:length(control_surface_states)
    aircraft=aircraft.f_set_control_surface(aircraft.control_surfaces{n_ctrl},control_surface_states_0{n_ctrl});
end
aircraft=aircraft.compute_grid();
...
for i=1:length(t_vec)
    ...
    XHALE_aero=XHALE_aero.solve_time_domain_aerodynamics(aircraft,[([-1 0 0;0 1 0; 0 0 -1]*MeanAxisModal.Xe(i,:)')' MeanAxisModal.Euler(i,:)],MeanAxisModal.V(i),MeanAxisModal.Alpha(i),-MeanAxisModal.Beta(i),...
        [-1 0 0;0 1 0; 0 0 -1]*MeanAxisModal.pqr(i,:)',rho_air,i,['results/' aircraft.name '/' maneuver_name '/movie/mean_axis_modal_' num2str(n_modes) 'modes' ]);
    
    
    XHALE_aero.F_body(1,:)=-sin(MeanAxisModal.Alpha(i)).*XHALE_aero.F_body(3,:);
    ...
    % no structural damping
        ...
        MeanAxisModal.Error(i)=norm(err_vec);
...
MeanAxisModal.CX=XHALE_aero.CX;
MeanAxisModal.CY=XHALE_aero.CY;
MeanAxisModal.CL=XHALE_aero.CL;
MeanAxisModal.CZ=XHALE_aero.CZ;
MeanAxisModal.CM=XHALE_aero.CM;
MeanAxisModal.CN=XHALE_aero.CN;
MeanAxisModal.x_AE=x_AE;
save('MeanAxisModal','MeanAxisModal','MeanAxisModal');
%save('AeroelasticModalStates','x_AE','x_AE')
save(['results/' aircraft.name '/' maneuver_name '/Simulation_Mean_Axis_Modal' num2str(n_modes) 'modes'],'MeanAxisModal','MeanAxisModal');
