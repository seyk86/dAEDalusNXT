...
%
function MeanAxisHybrid=simulate_free_flying_hybrid_static_dynamic(aircraft,aircraft_structure,AeroelasticSSM,trimmed_state,ctrl_input,t_vec,dt,maneuver_name,movie_on)
...
x_AE=zeros(length(t_vec),length(AeroelasticSSM.ASee));
x_AE_0=zeros(length(t_vec),length(AeroelasticSSM.ASee));
x_AE_dot=zeros(length(AeroelasticSSM.ASee),1);
...
%load(['results/' aircraft.name '/' maneuver_name '/Simulation_Mean_Axis']);
x_AE(1,:)=AeroelasticSSM.ASee^(-1)*(-AeroelasticSSM.ASre(:,:)*[0 0 0 0 MeanAxisHybrid.Alpha(1)  MeanAxisHybrid.Beta(1)   ([135 0 0]+(Tm*MeanAxisHybrid.Vb(1,:)')')*0   MeanAxisHybrid.pqr(1,:)*0 zeros(1,6)]'-AeroelasticSSM.ASce*(ctrl_state_0')*pi/180);
...
aircraft.write_grid_deflected([aircraft.name '/Trim_Shape_Rogers'],[0 0 0]',[0 alpha_k  0]'*pi/180,[0 0 0]');
trim_shape=zeros(length(aircraft_structure.modefrequencies),1);
aircraft_structure.nodal_deflections=trim_shape(:);
aircraft_structure=aircraft_structure.f_postprocess();
rho_air=trimmed_state.aerodynamic_state.rho_air;

x_stat=AeroelasticSSM.ASee^-1*1/2*trimmed_state.aerodynamic_state.rho_air*trimmed_state.aerodynamic_state.V_A^2*aircraft.reference.S_ref*[zeros(n_elastic_modes,1);AeroelasticSSM.Q_0(7:6+n_elastic_modes,1);zeros(size(AeroelasticSSM.ASre(:,:),1)-2*n_elastic_modes,1)];
for i=1:length(t_vec)
    ...
    VIn=-((Tm*MeanAxisHybrid.Vb(i,:)')'-(Tm*MeanAxisHybrid.Vb(1,:)')');
    VIn(3)=VIn(3)+(MeanAxisHybrid.Alpha(i)-sin(MeanAxisHybrid.Alpha(1)))*MeanAxisHybrid.Vb(i,1);
    
    x_AE_0(i,:)=AeroelasticSSM.ASee^(-1)*(-AeroelasticSSM.ASre(:,:)*[0 0 0 0 MeanAxisHybrid.Alpha(i)  MeanAxisHybrid.Beta(i)   ([135 0 0]+(Tm*MeanAxisHybrid.Vb(1,:)')')*0   MeanAxisHybrid.pqr(i,:) zeros(1,6)]'-AeroelasticSSM.ASce*((ctrl_state+ctrl_state_0)')*pi/180);
    xAE=x_AE(i,:)'; 
    for its=1:30 % fixed point iteration
        x_AE_dot=AeroelasticSSM.ASee*(xAE)+AeroelasticSSM.ASce*((ctrl_state+ctrl_state_0)')*pi/180 ...
            +AeroelasticSSM.ASre*[0 0 0 0 AlphaIn  MeanAxisHybrid.Beta(i)*0  VIn  (T*MeanAxisHybrid.pqr(i,:)'*0)' 0 0 0 0 0 0  ]';
    ...
    F_el=-AeroelasticSSM.Cer*(x_AE_0(i,1:length(AeroelasticSSM.Cer))'+x_stat(1:length(AeroelasticSSM.Cer)))*0;
