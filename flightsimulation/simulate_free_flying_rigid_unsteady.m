...
%
function MeanAxisHybrid=simulate_free_flying_rigid_unsteady(aircraft,aircraft_structure,trimmed_state,ctrl_input,t_vec,dt,maneuver_name,movie_on)
...
for i=1:length(t_vec)
   ...
   aircraft_aero=aircraft_aero.solve_time_domain_aerodynamics(aircraft,[([-1 0 0;0 1 0; 0 0 -1]*MeanAxisHybrid.Xe(i,:)')' MeanAxisHybrid.Euler(i,:)],MeanAxisHybrid.V(i),MeanAxisHybrid.Alpha(i),-MeanAxisHybrid.Beta(i),...
    ...
    if mod(i,rtf)==0
        ...
        rigid_forces(i/rtf,1)=aircraft_aero.qinf*aircraft_aero.CX(i/rtf);
        rigid_forces(i/rtf,2)=aircraft_aero.qinf*aircraft_aero.CY(i/rtf);
        rigid_forces(i/rtf,3)=aircraft_aero.qinf*aircraft_aero.CZ(i/rtf);
        rigid_forces(i/rtf,4)=aircraft_aero.qinf*aircraft_aero.CL(i/rtf);
        rigid_forces(i/rtf,5)=aircraft_aero.qinf*aircraft_aero.CM(i/rtf);
        rigid_forces(i/rtf,6)=aircraft_aero.qinf*aircraft_aero.CN(i/rtf);
        MeanAxisHybrid.cF(i/rtf,:)=F;
        MeanAxisHybrid.cM(i/rtf,:)=M;
    end
    ...
    if i==1
        ...
        MeanAxisHybrid.cF(1,:)=F;
        MeanAxisHybrid.cM(1,:)=M;
