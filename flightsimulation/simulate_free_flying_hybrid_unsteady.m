...
%
function MeanAxisHybrid=simulate_free_flying_hybrid_unsteady(aircraft,aircraft_structure,AeroelasticSSM,trimmed_state,ctrl_input,t_vec,dt,maneuver_name,movie_on)
...
x_AE(1,:)=AeroelasticSSM.ASee^(-1)*(-AeroelasticSSM.ASre(:,:)*[0 0 0 0 MeanAxisHybrid.Alpha(1)*0  MeanAxisHybrid.Beta(1)   ([135 0 0]+(Tm*MeanAxisHybrid.Vb(1,:)')')*0   MeanAxisHybrid.pqr(1,:)*0 zeros(1,6)]'-AeroelasticSSM.ASce*(ctrl_state_0')*0*pi/180);
...
x_stat=0*AeroelasticSSM.ASee^-1*1/2*trimmed_state.aerodynamic_state.rho_air*trimmed_state.aerodynamic_state.V_A^2*aircraft.reference.S_ref*[zeros(n_elastic_modes,1);AeroelasticSSM.Q_0(7:6+n_elastic_modes,1);zeros(size(AeroelasticSSM.ASre(:,:),1)-2*n_elastic_modes,1)];
...
for i=1:length(t_vec)
    ...
    for its=1:30 % fixed point iteration
        x_AE_dot=AeroelasticSSM.ASee*xAE+AeroelasticSSM.ASce*((ctrl_state+0*ctrl_state_0)')*pi/180 ...
            +AeroelasticSSM.ASre*[0 0 0 0 AlphaIn  MeanAxisHybrid.Beta(i)*0  VIn  (T*0*MeanAxisHybrid.pqr(i,:)')' 0 0 0 0 0 0  ]';
    ...
    F_el=-AeroelasticSSM.Cer*0*((x_AE(i,1:length(AeroelasticSSM.Cer))')+x_stat(1:length(AeroelasticSSM.Cer)));
...
%        -aircraft_aero.qinf*aircraft_aero.CZ*aircraft_aero.reference.S_ref+gravity(3)*Mass(1,1)];
    ...
    F=[aircraft_aero.qinf*aircraft_aero.Cl*sin(MeanAxisHybrid.Alpha(i))*aircraft_aero.reference.S_ref+gravity(1)*Mass(1,1);
       aircraft_aero.qinf*aircraft_aero.CY*aircraft_aero.reference.S_ref+gravity(2)*Mass(1,1);
       -aircraft_aero.qinf*aircraft_aero.Cl*cos(MeanAxisHybrid.Alpha(i))*aircraft_aero.reference.S_ref+gravity(3)*Mass(1,1)];
