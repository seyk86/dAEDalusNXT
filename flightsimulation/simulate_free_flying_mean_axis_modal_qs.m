...
%
function MeanAxisModal=simulate_free_flying_mean_axis_modal_qs(aircraft,aircraft_structure,n_modes,trimmed_state,ctrl_input,t_vec,dt,maneuver_name,movie_on)
...
MeanAxisModal.Loads=zeros(ceil(length(t_vec)/rtf),length(aircraft_structure.Ftest));
MeanAxisModal.Error=zeros(length(t_vec),1);
% for testing
gravity_on=1;
...
for i=1:length(t_vec)
    ...
    MeanAxisModal.Beta(i)=atan(-MeanAxisModal.Vb(i,2)/MeanAxisModal.Vb(i,1));
    MeanAxisModal.V(i)=norm(MeanAxisModal.Vb(i,:));                  
    [T, a, P, rho_air] = atmosisa(-MeanAxisModal.Xe(i,3));      
    ...
    mean_axis_origin=-[sum(aircraft_structure.Mff_lumped(1:6:end,1:6:end)*(aircraft_structure.node_coords(:,1))+aircraft_structure.nodal_deflections(1:6:end))/aircraft.weights.W;
                      sum(aircraft_structure.Mff_lumped(2:6:end,2:6:end)*(aircraft_structure.node_coords(:,2))+aircraft_structure.nodal_deflections(2:6:end))/aircraft.weights.W;
                      sum(aircraft_structure.Mff_lumped(3:6:end,3:6:end)*(aircraft_structure.node_coords(:,3))+aircraft_structure.nodal_deflections(3:6:end))/aircraft.weights.W];
    ...
    [M_tot_mean,K_tot_mean,F_acc_mean]=compute_mean_axis_modal_matrices(A_Bar,I_Hat,b,b_Hat_Skew,omega_Hat_Skew,aircraft_structure.Mff,aircraft_structure.Kff,aircraft_structure.modeshapes,n_modes,xdot_now);
    ...
    aero_state=class_aero_state(MeanAxisModal.V(i),MeanAxisModal.Alpha(i)*180/pi,-MeanAxisModal.Beta(i)*180/pi,trimmed_state.aerodynamic_state.Ma,rho_air);
    aircraft_aero=aircraft_aero.f_set_state(aero_state);
    aircraft_aero=aircraft_aero.f_solve_dynamic(-MeanAxisModal.pqr(i,1),MeanAxisModal.pqr(i,2),-MeanAxisModal.pqr(i,3),deflection_induced_speed);
    MeanAxisModal.CX(i)=aircraft_aero.CX;
    MeanAxisModal.CZ(i)=aircraft_aero.CZ;
    MeanAxisModal.CM(i)=aircraft_aero.CM;
    MeanAxisModal.CY(i)=aircraft_aero.CY;
    MeanAxisModal.CL(i)=aircraft_aero.CL;
    MeanAxisModal.CN(i)=aircraft_aero.CN;
    
    %aircraft_aero=aircraft_aero.solve_time_domain_aerodynamics(aircraft,[([-1 0 0;0 1 0; 0 0 -1]*MeanAxisModal.Xe(i,:)')' MeanAxisModal.Euler(i,:)],MeanAxisModal.V(i),MeanAxisModal.Alpha(i),MeanAxisModal.Beta(i),...
   %     [-1 0 0;0 1 0; 0 0 -1]*MeanAxisModal.pqr(i,:)',rho_air,i,['results/' aircraft.name '/' maneuver_name '/movie/mean_axis_modal']);
    ...
    F_tot_mean=F_acc_mean+[I_Hat'*A_Bar*aircraft_structure.Ftest;b_Hat_Skew'*A_Bar*aircraft_structure.Ftest;Fm];

    % no structural damping
    C_mean=K_tot_mean*0.001+M_tot_mean*0.001;
    
    if mod(i,rtf)==0
        MeanAxisModal.Loads(i/rtf,:)=aircraft_structure.Ftest+modeshapes_rom(1:end,7:end)*F_acc_mean(7:end);
    end
...
MeanAxisModalQS=MeanAxisModal;
save('MeanAxisModal','MeanAxisModal','MeanAxisModal');
%save('AeroelasticModalStates','x_AE','x_AE')
save(['results/' aircraft.name '/' maneuver_name '/Simulation_Mean_Axis_ModalQS'],'MeanAxisModalQS','MeanAxisModalQS');
